...
export function CreateVirtualMachine(_: CreateVirtualMachineProps) {
    ...
    useEffect(() => {
        ...
        client.listNetworks(request, null, (err, res) => {
            ...
            setNetworkList([...res.getNetworksList()]);
    ...
    const [blockStorageMap, setBlockStorageMap] = useState(
        {} as { [key: string]: BlockStorage }
    );
    const [blockStorageList, setBlockStorageList] = useState(
        [] as BlockStorage[]
    );
    useEffect(() => {
        ...
        client.listBlockStorages(request, null, (err, res) => {
            ...
            const bss = res.getBlockStoragesList().filter((v) => {
                return (
                    v.getState() === BlockStorage.BlockStorageState.AVAILABLE
                );
            });
    ...
    const onChangeSelectedNodeName = (
    ...
    const onChangeSelectNetworkList = (
        e: React.ChangeEvent<HTMLSelectElement>
    ) => {
    ...
    const [newBlockStorages, setNewBlockStorages] = useState(
    ...
    const onClickAddBS = () => {
        ...
        const imageTag =
            imageList.length > 0
                ? imageList[0].getTagsMap().toArray()[0][0]
                : "";
        const newBS = {
            name: "",
        ...
        setNewBlockStorages([...newBlockStorages, newBS]);
    };
    ...
    const onClickCreateVirtualMachine = () => {
        ...
        vm.setBlockStorageNamesList(
            newBlockStorages.map((s) => {
                return s.name;
            })
        );
        ...
        newBlockStorages.forEach((s) => {
            if (s.type === SelectBlockStorageType.FromImage) {
                ...
                generateBlockStorageRequest
                    .getAnnotationsMap()
                    .set(
                        "n0core/provisioning/block_storage/request_node_name",
                        selectedNodeName
                    );
                console.log(generateBlockStorageRequest.getAnnotationsMap());
                const client = new ImageServiceClient(Config.ProxyURL, {});
                client.generateBlockStorage(
                    generateBlockStorageRequest,
                    null,
                    (err, res) => {
                        console.log(err);
        ...
        request
            .getAnnotationsMap()
            .set(
                "n0core/provisioning/virtual_machine/request_node_name",
                selectedNodeName
            );
    ...
    return (
        <Container>
            <Typography variant="h4">Create VirtualMachine</Typography>
            <div>
                ...
                >
                    {nodeList.map((n, key) => (
                        <option key={key} value={n.getName()}>
                            {n.getName()}
                        </option>
                    ))}
                </NativeSelect>
            ...
            <div>
                <TextField label="Name" onChange={onChangeNewVMName} />
            </div>
            <div>
                <TextField label="vcpus" onChange={onChangeNewVMVcpus} />
            </div>
            <div>
                <TextField label="memory(GB)" onChange={onChangeNewVMMemory} />
            </div>
            <div>
                <TextField label="user" onChange={onChangeUserName} />
                <TextField
            ...
            <div>
                ...
                {vm.getNicsList() !== null &&
                    vm.getNicsList().map((nic, key) => {
                        const onChangeIPv4Address = (
                        ...
                        return (
                            <Card key={key} className={classes.card}>
                                ...
                                <CardActions>
                                    ...
                                    >
                                        <Button onClick={onSwapUp}>↑</Button>
                                        <Button onClick={onSwapDown}>↓</Button>
                                    </ButtonGroup>
                                    <Button
                                        variant="outlined"
                                        color="secondary"
            ...
            <div>
                ...
                {newBlockStorages.map((bs, key) => {
                    const onChangeBSType = (
                        e: React.ChangeEvent<HTMLInputElement>
                    ) => {
                        const newBSS = [...newBlockStorages];
                        newBSS[key].type = e.target
                            .value as SelectBlockStorageType;
                        setNewBlockStorages(newBSS);
                    };
                    const onChangeBSName = (
                        e: React.ChangeEvent<HTMLInputElement>
                    ) => {
                        const newBSS = [...newBlockStorages];
                    ...
                    const onDelete = () => {
                        const newBSs = [
                            ...newBlockStorages.filter((_, i) => i !== key),
                        ];
                        setNewBlockStorages(newBSs);
                    };
                    const onSwapUp = () => {
                                ...
                                {bs.type === SelectBlockStorageType.FromImage &&
                                    (() => {
                                        const onChangeImageName = (
                                            ...
                                            newBSS[key].imageName =
                                                e.target.value;
                                            const imageTag = imageList
                                                .find(
                                                    (i) =>
                                                        i.getName() ===
                                                        e.target.value
